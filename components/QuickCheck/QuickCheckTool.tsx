...
const QuickCheckTool: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await analyzeOrganization(ein);
      setResult(data);
    } catch (err) {
  ...
  if (result) {
    return (
        <div className="w-full max-w-4xl mx-auto px-4" id="quick-check-results">
            <ResultsCard 
              result={result} 
              onReset={resetTool} 
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 relative z-10" id="quick-check">
